...
import { type FlowchartNode_CustomScript } from '../types/script3'
import General_CustomScriptAndCondition from './General_CustomScriptAndCondition.vue'

// props were passed from the slot using `v-bind="customNodeProps"`
const props = defineProps<NodeProps<FlowchartNode_CustomScript['data']>>()
</script>
...
<template>
  ...
  <div>
    ...
    <div>
      <div>
        <Icon class="custom-node-icon"><Javascript24Regular /></Icon>
        <span class="custom-node-title"
          ><u v-tooltip.top="$t('comp.flowchart.aliya2_demo.node.CustomScript.title.tooltip')">{{
            $t('comp.flowchart.aliya2_demo.node.CustomScript.title')
          }}</u></span
        >
      </div>
