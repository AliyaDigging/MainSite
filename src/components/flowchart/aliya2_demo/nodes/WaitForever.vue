...
import { StopCircleOutlined } from '@vicons/material'

import { type FlowchartNode_WaitForever } from '../types/script3'
import General_CustomScriptAndCondition from './General_CustomScriptAndCondition.vue'

// props were passed from the slot using `v-bind="customNodeProps"`
const props = defineProps<NodeProps<FlowchartNode_WaitForever['data']>>()
</script>
...
<template>
  ...
  <div>
    ...
    <div>
      <div>
        <Icon class="custom-node-icon"><StopCircleOutlined /></Icon>
        <span class="custom-node-title"
          ><u v-tooltip.top="$t('comp.flowchart.aliya2_demo.node.WaitForever.title.tooltip')">{{
            $t('comp.flowchart.aliya2_demo.node.WaitForever.title')
          }}</u></span
        >
      </div>
