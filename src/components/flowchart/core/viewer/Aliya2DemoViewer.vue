...
import Variable_UsagePopover from '../../aliya2_demo/nodes/Variable_UsagePopover.vue'
...
type FlowchartMetadata = FlowchartData_Aliya2Demo['metadata']
...
provide(symbolFlowchartMetadata_Aliya2Demo, vueflowData.metadata)
...
async function fitInitialNode() {
  if (vueflowData.nodes.value.length > 0) {
    const initialNode = vueflowData.nodes.value[1] ?? vueflowData.nodes.value[0]
    await vueflow.fitView({ nodes: [initialNode.id] })
  }
}
  ...
  async (newValue) => {
    if (newValue.endsWith('/.json') || !props.flowchartName) {
      ...
      vueflowData.metadata.value = {
        ...
        currName: '',
        currId: '',
        articyInternal: { title: '', description: '', articyId: '', channelId: null },
      }
    } else {
      const cached = store.getCachedState(flowKey.value)
      if (cached) {
