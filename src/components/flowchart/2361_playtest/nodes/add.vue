...
import { type FlowchartDataNode_add } from '../types/script3'

// props were passed from the slot using `v-bind="customNodeProps"`
const props = defineProps<NodeProps<FlowchartDataNode_add['data']>>()
</script>
...
<template>
  ...
  <div>
    ...
    <div>
      <div>
        <Icon class="custom-node-icon"><Add12Filled /></Icon>
        <span class="custom-node-title">{{ $t('comp.flowchart.node.add.title') }}</span>
      </div>
      <div class="custom-node-content">
        ...
        <ul class="custom-node-normal-ul">
          <li v-for="(item, idx) in props.data.arg.vars" v-bind:key="idx">
            <code>{{ item.varName }} + {{ item.varAddValue }}</code>
          </li>
        </ul>
        <p>
          延迟时间:
