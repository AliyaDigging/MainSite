...
import { Slideshow } from '@vicons/tabler'

import { type FlowchartDataNode_shot } from '../types/script3'

// props were passed from the slot using `v-bind="customNodeProps"`
const props = defineProps<NodeProps<FlowchartDataNode_shot['data']>>()
</script>
...
<template>
  ...
  <div>
    ...
    <div>
      <div>
        <Icon class="custom-node-icon"><Slideshow /></Icon>
        <span class="custom-node-title">{{ $t('comp.flowchart.node.shot.title') }}</span>
      </div>
      <div class="custom-node-content">
        ...
        <p>
          图片ID: <code>{{ props.data.arg.img }}</code>
        </p>
        <p>
          延迟时间:
          {{
